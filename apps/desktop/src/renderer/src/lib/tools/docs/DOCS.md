...
`TranslateDrag.shiftKey` follows the processed drag sample. Translation previews run in `onStateEnter` after the sample is published, so the snap predicate reads the current tool state rather than a stale event context or global keyboard modifiers. This also preserves the final-release modifier contract. The preview's effective delta is then published into `totalDelta`; cancellation and commit continue through the existing interaction rollback scope.
