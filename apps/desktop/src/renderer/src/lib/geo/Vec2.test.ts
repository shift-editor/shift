...
describe('Vec2', () => {
  ...
  describe('geometric operations', () => {
    it('mirrors a point across an anchor', () => {
      expect(Vec2.mirror({ x: 3, y: 5 }, { x: 0, y: 0 })).toEqual({ x: -3, y: -5 });
      expect(Vec2.mirror({ x: 4, y: 6 }, { x: 2, y: 3 })).toEqual({ x: 0, y: 0 });
    });
  ...
  describe('utility', () => {
    ...
    it('computes midpoint', () => {
      expect(Vec2.midpoint({ x: 0, y: 0 }, { x: 10, y: 20 })).toEqual({ x: 5, y: 10 });
    });
