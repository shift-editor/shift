...
- **Architecture Invariant:** `LayerBuffers.#pointContours` owns the current point-to-contour index. Successful point insertions and removals update it in the same reactive batch as geometry; rejected operations leave it unchanged. `fromState` rebuilds membership when snapshots replace topology, including rollback and active-edit reapplication. Coordinate, smoothness, closure, and winding edits preserve membership without rebuilding the index.
...
- **Architecture Invariant:** `GlyphLayerEdit` mutates the ordinary reactive layer during a cancelable structural interaction, so rendering, bounds, hit testing, and object ownership observe one complete current topology. It exposes only generic layer primitives such as adding contours, points, and anchors or setting smoothness and positions; tool semantics remain outside the model. Workspace replacements update its restoration base and reapply the active edit in the same batch. Finishing replays final typed operations through `GlyphLayerWriter`; canceling restores the latest accepted base.
- **Architecture Invariant:** `GlyphLayer.positions` creates operation-specific fluent edits backed by one active `GlyphLayerEdit`. Direct position operations lazily own that edit; `positions.within(edit)` binds one terminal position operation to an existing edit so structural and positional changes finish or cancel together. Configuration is legal only before the first preview; every preview resolves from one frozen position base, workspace replacements update the active layer-edit restoration base and reapply the latest preview, and completion either finishes one workspace edit or cancels to the latest accepted base. Tools may configure these edits but do not own their lifecycle mechanics.
