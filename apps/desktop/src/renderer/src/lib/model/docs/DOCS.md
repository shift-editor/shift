...
lib/model/
  ...
  Glyph.ts                   -- Glyph, GlyphLayer, GlyphView, geometry/render surfaces
  GlyphOutline.ts            -- recursive component expansion and lazy path construction
...
- `GlyphView` -- stable reactive view bound to a location signal. It can move between projection-backed and live-layer-backed geometry without changing consumer APIs.
- `GlyphOutline` -- derived composed outline that recursively asks `Font` for each component's current authored layer or projected geometry.
...
Only observed view output is evaluated. Virtualized offscreen views do not subscribe to paths, and no sequence of scrubbed locations increases retained geometry. Components reuse the same projection/layer resolution recursively.
