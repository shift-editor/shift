...
- **Architecture Invariant:** `Font.matchLayers()` reads Rust-owned `LayerMatch` values through the serialized workspace lane after pending edits. `Editor` retains results only for the current glyph, reference source, editing source set, and layer-structure identities; it preserves them across coordinate-only edits. The renderer never infers mappings or uses incomplete matches for partial edits.
