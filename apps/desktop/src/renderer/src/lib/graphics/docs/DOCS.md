...
- **Architecture Invariant:** Both session modes expose one immutable `GlyphCatalog`. The shared Grid consumes stable session `GlyphId` keys, external coordinates, catalog metrics, invalidation, and `GlyphAtlasSource`; backend GIDs remain private to the native session identity map. `openGlyph` always calls the common `Font.loadGlyph()` before navigation.

- **Architecture Invariant:** Catalog route visibility and GPU readiness are independent. Leaving `/home` makes the catalog controller inactive without replacing its active frame, destroying its resident layer, or resizing its canvas. Returning reactivates the controller and submits one repaint because Chromium may discard a hidden WebGPU presentation; it requires no atlas acquisition or page reconstruction. Every visible page is composited through one WebGPU render pass. Initial readiness requires the complete fixed-page set and a completed submitted frame; after that, scrolling performs no atlas acquisition or geometry upload.
