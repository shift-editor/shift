...
- **Architecture Invariant:** Authored glyphs use `Renderer`; retained `DisplayGlyph` values use `DisplayGlyphCanvas`. Both feed the same `Handles`, `ControlLines`, `Anchors`, `Guides`, `OutlineRenderer`, and `Canvas` primitives rather than maintaining format-specific drawing code. The retained host deliberately leaves `MarkerLayer` unavailable so `Handles` uses its existing Canvas 2D fallback instead of creating a second GPU context beside the resident Grid.
...
- **Architecture Invariant:** Catalog route visibility and GPU readiness are independent. Leaving `/home` makes the catalog DOM inert but does not deactivate its controller, replace its active frame, destroy its resident layer, or resize its canvas. Every visible page is composited through one WebGPU render pass, so returning to the already-painted Grid requires no atlas acquisition, page reconstruction, or delayed activation. Initial readiness requires the complete fixed-page set and a completed submitted frame; after that, scrolling performs no atlas acquisition or geometry upload.
