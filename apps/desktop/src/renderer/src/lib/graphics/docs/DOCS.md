...
- **Architecture Invariant:** `ResidentGlyphLayer` is the Slug catalog-preview boundary. It owns one WebGPU adapter/device/context and independently replaceable root pages from `GlyphAtlasSource`; authored and retained-source acquisition remain behind adapters, and packed-layout knowledge never enters the shared React catalog shell.
...
- `GlyphCatalogRenderer` -- session-fixed catalog update and lifetime contract
- `SlugGlyphCatalogRenderer` -- screen-space Slug interaction, atlas, and frame owner
- `SvgGlyphCatalogRenderer` -- complete-frame DOM/SVG fallback with bounded preview residency
