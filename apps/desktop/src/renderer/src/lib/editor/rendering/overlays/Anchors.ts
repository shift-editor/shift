...
export class Anchors {
  draw(
    canvas: Canvas,
    anchors: readonly GlyphRenderAnchor[],
    state: HandleStateSource,
  ): void {
    for (const anchor of anchors) {
      drawHandle(
        ...
        "anchor",
        this.#anchorState(anchor, state),
      );
    }
  }

  #anchorState(
    anchor: GlyphRenderAnchor,
    source: HandleStateSource,
  ): HandleState {
    if (source.selection.anchorIds.has(anchor.id)) return "selected";

    if (source.hover?.type === "anchor" && source.hover.anchorId === anchor.id)
      return "hovered";
