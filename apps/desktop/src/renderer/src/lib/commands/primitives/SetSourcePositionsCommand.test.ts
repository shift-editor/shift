...
import { SetSourcePositionsCommand } from "./SetSourcePositionsCommand";
...
describe("SetSourcePositionsCommand", () => {
  it("replays after positions and restores before positions", () => {
    const source = editableSource();
    const point = source.allPoints[0];
    if (!point) throw new Error("Expected point");

    const before = [{ kind: "point" as const, id: point.id, ...pointPosition(source, point.id) }];
    const after = [
      { kind: "point" as const, id: point.id, x: before[0].x + 15, y: before[0].y + 5 },
    ];
    const command = new SetSourcePositionsCommand("Move Selection", before, after);

    command.execute(ctx(source));
    expect(pointPosition(source, point.id)).toEqual({ x: after[0].x, y: after[0].y });

    command.undo(ctx(source));
    expect(pointPosition(source, point.id)).toEqual({ x: before[0].x, y: before[0].y });
  ...
  it("derives before positions from the source", () => {
    ...
    const command = SetSourcePositionsCommand.fromSource("Move Selection", source, after);
