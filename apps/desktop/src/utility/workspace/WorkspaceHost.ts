...
export class WorkspaceHost {
  ...
  #connectSyncLane(ports: readonly unknown[]): void {
    ...
    this.#sync = serveChannel<SyncCallMap, SyncEventMap>(this.#portTransport(port), {
      ...
      "source.snapshot": () =>
        this.#serialize(() => (this.#state.kind === "preview" ? this.#state.preview : null)),
  ...
  #openFontSource(sourcePath: string): FontSourceSession {
    ...
    this.#state = { kind: "preview", preview: { ...session, font } };
  ...
  #documentState(): WorkspaceDocumentState | null {
    if (this.#state.kind !== "document") return null;
  ...
  #requirePreview() {
    if (this.#state.kind !== "preview") {
      throw new Error("no preview is open");
    }
