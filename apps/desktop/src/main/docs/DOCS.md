...
- **Architecture Invariant:** `shared/workspace/channel.ts` owns only correlated calls/events over the required `Transport` shape. Electron, DOM and Node-port adapters live in `localTransports.ts`; WebSocket lanes use bounded MessagePack from `channelCodec.ts`, preserving `Uint8Array` as binary values and `Float64Array`/`undefined` through assigned extension codes. One ordinary frame is limited to 32 MiB, malformed or unsupported values close the socket, and WebSocket lanes reject transferred ports. Atlas byte streams remain on local transferred ports until a separately bounded network stream protocol lands.
...
- `shared/workspace/channelCodec.ts` -- bounded MessagePack envelope codec with typed-array preservation.
