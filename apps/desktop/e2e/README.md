...
- `expectCanvasSnapshot(editor, name)` waits for pending edits and two rendered frames, then compares the composited canvas stack (`editorCanvasStack()`) exactly, on every host. Exact means both `maxDiffPixels: 0` and `threshold: 0`: Playwright's default per-pixel threshold ignores colour changes smaller than 0.2, which let a changed translucent outline token pass unnoticed. Software canvas rendering is pixel-identical on development Macs and the hosted runner. Individual canvas layers are not captured separately: an element screenshot of one stacked layer includes the layers above it.
