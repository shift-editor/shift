...
import { asContourId, asPointId, type ContourData, type PointData } from "@shift/types";
import {
...
type TestPoint = PointData & { x: number; y: number };
type TestContour = ContourData & { points: TestPoint[] };

function makePoint(
  id: string,
  x: number,
  y: number,
  pointType: PointData["pointType"] = "onCurve",
) {
  return {
    id: asPointId(id),
    x,
...
function makeGlyph(input: { contours?: TestContour[] }): SegmentGlyphLike {
  return { contours: input.contours ?? [] };
}
