...
use super::{
    ...
    reader::{
        axis_mappings_from_designspace, axisless_source_name, find_default_source_index,
        location_from_dimensions, named_instances_from_designspace, parse_axisless_sources,
...
fn add_axes(
    ...
    for axis in &document.axes {
        let mut imported = match variation_axis_kind(axis) {
            VariationAxisKind::Discrete { values, default } => Axis::discrete_with_id(
                AxisId::new(),
                axis.tag.clone(),
                axis.name.clone(),
                values.into_vec(),
...
fn build_glyph_directory(
    ...
    let names = directories
        .iter()
        .flat_map(|directory| directory.glyphs.keys().cloned())
        .collect::<BTreeSet<_>>();
    let glyph_ids = names
        .iter()
    ...
    let glyphs = names
        .into_iter()
        .map(|name| {
            let layers = directories
                .iter()
                .filter_map(|directory| {
                    let path = directory.glyphs.get(&name)?;
                    Some(GlifLayer {
