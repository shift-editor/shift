...
use glyphs_reader::{FeatureSnippet, Font as GlyphsFont, Glyph as GlyphsGlyph, NodeType, Shape};
use shift_font::{
...
pub(super) fn font_header(
    ...
    for (master_index, master) in glyphs_font.masters.iter().enumerate() {
        ...
        for (axis_index, axis_id) in axis_ids_by_index.iter().enumerate() {
            if let Some(value) = master.axes_values.get(axis_index) {
...
pub(super) fn convert_glyph(
    ...
    for layer in &glyph.layers {
        ...
        for anchor in &layer.anchors {
            ...
            result_layer.add_anchor(Anchor::new(name, anchor.pos.x, anchor.pos.y));
        }
