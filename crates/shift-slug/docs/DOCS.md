...
For 150 uniformly sampled Source Han glyphs, worst-case scratch reservation is bounded by the visible curves and `curve_count × 16` temporary band-index slots rather than all 65,535 glyphs. General varying component evaluation, anchor attachments, variable advances, and full authored CJK import remain subsequent model layers.
...
The authored `shift-font` variable fixture matches native midpoint projection within 0.001 font units. Exact-source shape or relationship exceptions are retained as separate static resident glyphs; `AuthoredGlyph` selects them by source identity through a visible-instance update rather than fabricating correspondence or uploading geometry. Atlas insertion is transactional across the compatible default and all variants.

The real MutatorSans designspace has 49 glyphs. All 49 build, including 10 glyphs / 20 nested component occurrences whose component glyphs share the root interpolation basis and whose linear matrices are constant while translations vary. The adapter rejects varying rotation/scale/skew rather than assuming flattened source deltas remain exact. Four deduplicated interpolation bases / 21 weight slots produce 702 base curves, 2,211 stored delta curves, seven sparse indexes, and 203 source descriptors in 74,072 bytes. Across all seven authored source locations plus 17 deterministic multi-axis locations, maximum curve error is 0.000183 font units. Attachments, varying linear component transforms, and mismatched component bases remain explicit requirements for the general component model.

The real Host Grotesk variable UI font exercises 448 glyphs / 9,481 curves across `wght=100..900`: base, midpoint, and source GPU scratch readback have zero coordinate error, generated band membership matches the CPU oracle exactly, and endpoint/midpoint fragment checksums differ as expected. Sparse support leaves its packed atlas byte-identical at 477,860 bytes. A 150-glyph pass uses about 291 KiB of visible scratch after adding exact visible bounds.

A diagnostic compiled-outline sweep covers all 65,535 Source Han glyph IDs at `wght=250` and `900`. Because independently resolved callbacks disagree structurally for 3,068 glyphs, the sizing harness retains those source shapes as exact resident variants rather than claiming production correspondence. The resulting 68,603 atlas glyphs contain 5,817,677 base curves, 5,148,741 stored deltas, and 4,100 sparse indexes in 267,283,116 bytes (254.901 MiB): 1.099 MiB below the preferred 256 MiB gate and well below the 500 MiB stop. Build time is 836 ms and deterministic serialization is 272 ms on the Linux harness. `write_packed_chunks` emits 64 bounded 4 MiB writes without a contiguous packed copy, reducing measured peak RSS from 582.8 to 363.2 MiB; the consumer can drop the authored atlas immediately after residency.
...
The `9c0b6510` run includes the component fast path and exact visible-bound reduction, which adds 16 scratch bytes per visible glyph and a ninth compute storage binding. It built in 1.373 ms, completed GPU submit/readback in 5.775 ms, retained zero geometry uploads and 960 weight bytes, passed curve and band validation, and preserved checksum `c1cd8eb7631a65db`. Relative to the preceding `f5dc1281` run, p50 improved 4.5% while p95, p99, and max were 17.3%, 12.4%, and 15.0% higher. Treat these as run-to-run observations. The latest p95 retains 5.746 ms below the preferred 8.3 ms gate and p99 retains 11.939 ms below the 16.7 ms hard frame gate. Sparse lookup adds a tenth storage binding while preserving Host's resident bytes and llvmpipe checksum; its Metal timing remains to be recorded.
