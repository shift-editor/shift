...
The authored `shift-font` variable fixture matches native midpoint projection within 0.001 font units. The real MutatorSans designspace has 49 glyphs. All 49 now build, including 10 glyphs / 20 nested component occurrences whose component glyphs share the root interpolation basis and whose linear matrices are constant while translations vary. The adapter rejects varying rotation/scale/skew rather than assuming flattened source deltas remain exact. Four deduplicated interpolation bases / 21 weight slots produce 702 base curves, 2,289 delta curves, and 203 source descriptors in 75,608 bytes. Across all seven authored source locations plus 17 deterministic multi-axis locations, maximum curve error is 0.000183 font units. Attachments, varying linear component transforms, mismatched component bases, and exact-source topology variants remain explicit requirements for the general component model.

The real Host Grotesk variable UI font exercises 448 glyphs / 9,481 curves across `wght=100..900`: base, midpoint, and source GPU scratch readback have zero coordinate error, generated band membership matches the CPU oracle exactly, and endpoint/midpoint fragment checksums differ as expected. A 150-glyph pass uses about 291 KiB of visible scratch after adding exact visible bounds.
