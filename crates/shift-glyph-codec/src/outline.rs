...
impl OutlineEncoder {
    ...
    pub fn finish(self) -> Result<PackedGlyphOutline, CodecError> {
        ...
        write_frame(&mut bytes, OUTLINE_KIND, OUTLINE_VERSION);
        bytes[8..12].copy_from_slice(&command_count.to_le_bytes());
        bytes[12..16].copy_from_slice(&coordinate_count.to_le_bytes());
...
pub fn decode_outline(bytes: &[u8]) -> Result<OutlineView<'_>, CodecError> {
    validate_frame(bytes, HEADER_LEN, OUTLINE_KIND, OUTLINE_VERSION).map_err(CodecError::from)?;
