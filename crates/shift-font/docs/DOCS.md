...
`Font::projection(location)` expects an internal authoring location. Apply external axis mappings before constructing it. For each glyph, the first variant whose condition matches the root font location selects its complete source set; no Default source fallback is allowed after that selection. Otherwise Default is selected. A glyph source combines its optional global-source location with sparse glyph-local overrides, and Fontdrasil evaluates the resulting global-plus-local sample locations. Layer-only/background global sources never contribute evaluated geometry.

Each active component overlays its sparse location on either the effective parent location (`AxisInheritance::Parent`) or the root font location (`AxisInheritance::Font`). Component conditions always evaluate against the root font location. Corresponding component transforms and explicit location values interpolate with the parent glyph source layers before the child resolves. Occurrences then flatten through the same authored ordering, transform, anchor-attachment, and branch-local cycle semantics as static components.
...
Transport and workspace layers should pass stable identity to find the model object, then call these methods. Glyph axes, glyph sources, and glyph variants use Create/Update/Delete intents backed by `Font::{add,replace,remove}_glyph_*`; Update is a full replacement that preserves identity, owner, and authored order, while variant membership changes use glyph-source operations. Component Add/Remove/Set intents persist as the existing `LayerGeometryReplaced` change. They should not introduce hidden native edit sessions.
